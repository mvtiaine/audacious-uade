...
int SongEndDetector::detect_silence(int seconds) {
    TRACE2("MAXI %d MINI %d\n", maxi, mini);
    const auto SAMPLES_PER_SEC = rate / 2;
    int songlen = volume_detect(buf, SAMPLES_PER_SEC * SILENCE_TIMEOUT, THRESHOLD_SILENCE) * 1000 / SAMPLES_PER_SEC;
    if (songlen) {
...
int SongEndDetector::detect_volume() {
    TRACE2("MAXI %d MINI %d\n", maxi, mini);
    const auto SAMPLES_PER_SEC = rate / 2;
    int songlen = volume_detect(buf, SAMPLES_PER_SEC * SILENCE_TIMEOUT, THRESHOLD_VOLUME) * 1000 / SAMPLES_PER_SEC;
    if (songlen) {
