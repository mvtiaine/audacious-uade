...
static bool GetPatternLength(uint16_t Rows, uint16_t *LengthOut)
{
	...
	for (int32_t i = 0; i < Rows; i++)
	{
		...
		for (int32_t j = 0; j < MAX_HOST_CHANNELS; j++, Src += 5, Enc += 6)
		{
			...
			uint16_t EfxAndParam = (uint16_t)((uint8_t)Src[3] | ((uint8_t)Src[4] << 8)); // mvtiaine: fixed UB
			if (EfxAndParam != 0)
			{
...
static void EncodePattern(pattern_t *p, uint8_t Rows)
{
	...
	for (int32_t i = 0; i < Rows; i++)
	{
		...
		for (uint8_t ch = 0; ch < MAX_HOST_CHANNELS; ch++, Src += 5, Enc += 6)
		{
			...
			uint16_t EfxAndParam = (uint16_t)((uint8_t)Src[4] << 8) | (uint8_t)Src[3]; // mvtiaine: fixed UB
			if (EfxAndParam != 0)
			{
			...
			if (Mask & 8)
			{
				memcpy(Dst, &EfxAndParam, sizeof(uint16_t)); // mvtiaine: fixed UB
				Dst += 2;
			}
		}
